<template>
  <div class="image-container">
    ...
    <el-dialog title="提示" :visible.sync="dialogVisible" width="700px">
      ...
      <!-- label="配置管理"选项卡的文字  el-tab-pane内容就是对应的内容容器-->
      <el-tabs v-model="activeName" type="card">
        <el-tab-pane label="素材库" name="image" v-loading>
          <!-- 单选框 -->
          <div>
...
export default {
  ...
  data () {
    return {
      // 默认 为 默认图片
      // 这个位置是否可以使用本地图片地址
      // 项目是webpack打包，如果本地的资源地址,存储在数据中心，是不会议打包到你项目中的。
      // 主动导入图片，此时图片资源就是一项数据 (base64的数据),
      // value:defaultImage
      defaultImage,
      // 上传图片的头部
      headers: {
  ...
  methods: {
    ...
    selected (url) {
      // 给当前点击的图片 加上selected
      //  :class="{selected:条件}"
      // 条件:根据当前图片的url 去匹配遍历的时候url一致加  不一致不加
